...
class _SignupPageState extends State<SignUpPage> {
  ...
  Future<void> _loadCurriculaNames() async {
    List<String> fetchedNames = await fetchCurriculaNames();
    setState(() {
      _dept.addAll(fetchedNames);
    });
  }
  ...
  void _guessAndSetRole() {
    ...
    final guessedYear = guessAdmissionYearFromStudentEmail(
  ...
  Future<List<String>> fetchCurriculaNames() async {
    final List<dynamic> data = await supabase.from('curricula').select('name');
...
class _EmailFormFieldState extends State<EmailFormField> {
  ...
  void _guessAndSetRole() {
    ...
    if (guessedRole != _currentRole ||
        guessedDepartment != _currentDepartment) {
...
class SignUpButtonText extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const Text('Sign Up', style: TextStyle(fontSize: 16));
  }
}
...
class AlreadyHaveAccountText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return TextButton(
      onPressed: () {
        // Wrap the method call in a function
